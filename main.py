...
def simulate(times = 1000, num_players = 3, setup_mode = 'random', expect_result = 'first_one', simulation_path:list = [], setup_tile_args:tuple = tuple(), setup_player_order_args:list = []):
    ...
    for _ in tqdm(range(times), desc="模拟进度"):
        game_args_dict = {
            ...
            'path_length': len(simulation_path),
            'action_mode': 'simulate',                          # input | simulate | reproduce
            'web_io': Silence_IO(),
            'need_estimate': False,
        }
    ...
    table_picture = table(analysed_result_list)
...
def normal(num_players = 3, setup_mode = 'random', setup_tile_args:tuple = tuple(), setup_player_order_args:list = [], need_estimate = False):
    ...
    match setup_mode:
        ...
        case 'input':
            pass
        case _:
            raise Exception('在常规游戏中不合法的初始设置模式（合法模式仅有input、random和target)')
...
if __name__ == "__main__":
    ...
    # simulate(times=1000,num_players=3)
    ...
    normal(num_players = 3, setup_mode = 'target', setup_tile_args = setup_tile_args, setup_player_order_args = [2,0,1])
