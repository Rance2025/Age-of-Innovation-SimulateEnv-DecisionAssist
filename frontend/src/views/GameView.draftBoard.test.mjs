...
test('draft board items render selected-player marker hooks and unavailable state hooks', () => {
  ...
  const imageStyles = cssBlock('.draft-item-image')
  const unavailableImageStyles = cssBlock('.draft-item.is-unavailable .draft-item-image')
  ...
  assert.match(imageStyles, /transition:[^;]*transform\s+0\.18s\s+ease/)
  assert.match(imageStyles, /transform-origin:\s*center/)
  assert.match(imageStyles, /border-radius:\s*6px/)
  assert.match(unavailableImageStyles, /transform:\s*scale\(0\.85\)/)
