...
class GamePanel:
    ...
    def output(self, channel, message):
        """2. 输出到指定信息框 (0=可选行动, 1+=玩家)"""
        if 0 <= channel <= self.player_count:
            self.queues['outputs'][channel].put(message)

    def update_player_states(self, states):
        """3. 更新所有玩家状态 (一次性更新)"""
        if isinstance(states, list) and len(states) == self.player_count:
            self.queues['player_states'].put(json.dumps(states))
