...
- 更新内容：
  - `ui: 将对局状态卡从可选行动列移至行动记录列顶部，并在原位置新增“控制中台”占位卡片，预留后续扩展空间。`
  - `fix: 对局页可选行动空态文案改为优先根据 is_game_over 标志判断，终局时不再显示“等待后端推送新的可选行动”。`
  - `feat: 对局状态卡在游戏结束时新增“查看最终比分”按钮，并在前端接入 final_scores 的全量、增量和 game_over 事件同步。`
  - `feat: 为对局页新增最终比分弹窗，支持按玩家展示总分、板块、连锁、轨道与资源分项。`
  - `ui: 仅下移可选行动区与行动记录区的空态提示块位置，不影响正常行动项与记录项的显示间距。`
  - `ui: 去掉行动记录筛选空态文案末尾句号，并将终局时可选行动区的提示统一精简为“本局已结束”。`
  - `ui: 进一步压缩对局状态卡的内边距、文案行高与按钮高度，同时增加控制中台占位区的预留高度。`
  - `ui: 重排对局状态卡头部与内容区布局，改为标题与菜单按钮同排、状态文案与终局操作按钮同排，提升整体对齐与排布稳定性。`
  - `ui: 去掉可选行动区与行动记录区 panel-empty-state 空态提示块的边框和背景色，仅保留文案与留白。`
  - `ui: 将对局状态卡右上角菜单按钮改为无边框无底色的纯白图标样式，弱化装饰性。`
  - `ui: 将行动记录头部计数胶囊与筛选按钮调整为与历史记录页筛选按钮一致的视觉语言，并统一筛选激活态反馈。`
  - `ui: 固定可选行动头部三枚状态胶囊的最小宽度与单行排布，避免终局时因文案变化产生位置抖动。`
  - `ui: 将行动记录头部两枚控件与可选行动头部三枚胶囊统一为原尺寸的扁平灰调胶囊样式，仅调整圆角与灰阶区分度，不增加立体阴影。`
  - `ui: 重新规划对局页头部 5 枚胶囊的尺寸与布局，统一为固定宽高的扁平灰调控件，并按 3 枚/2 枚栅格重排可选行动与行动记录头部。`
  - `ui: 修复可选行动区与行动记录区在窄宽度下标题和说明被头部胶囊挤压成竖排行的问题，并同步收紧胶囊尺寸与换行布局。`
  - `ui: 将可选行动头部的 3 枚状态胶囊改回左对齐，仅保留行动记录头部工具区的右对齐。`
  - `ui: 将行动记录头部的 2 枚胶囊进一步收窄并固定为与标题同排，避免再被挤到下一行。`
  - `ui: 进一步收缩可选行动头部 3 枚胶囊的宽高，并增大其与上方说明之间的纵向间距，使布局更接近参考图。`
  - `ui: 进一步缩小对局页头部胶囊的宽高与内部留白，在保持当前文字大小不变的前提下减轻内容周围空心感。`
  - `ui: 将行动记录计数胶囊单独加宽到可稳定容纳三位数，并把筛选按钮激活态与提示徽标改为更明显的蓝色高亮。`
  - `docs: 在 Agent 规范中补充默认版本号递增规则，若用户未强调版本号，则仅将最后一段编号加 1；本次版本为 0.9.5.11。`
- 验证方式：
